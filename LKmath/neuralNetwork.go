...
//
//				if i == 0{
//					temp[i].Cell[j][exampleNo] = YHat(inputMatrix,this.LayerParameter[0].LayerParameter[j]).Cell[0][exampleNo]
//
//				}else{
//					temp[i].Cell[j][exampleNo] = YHat(temp[i-1],this.LayerParameter[i].LayerParameter[j]).Cell[0][exampleNo]
//
//				}
//
//			}
...
//
//func UpdateFinalLayerDerivative(temp Matrix, y Matrix, nn NeuralNetwork)(NeuralNetwork, BackDerivative Matrix){
//
//	backTemp :=NewEmptyMatrix(nn.OutputLayerNum,y.Column)
//	for i :=0; i <  nn.OutputLayerNum; i++{
//		cm :=KeepOneRow(y, i)
//		d , backTempRow:=UpdateDerivative(temp,cm , nn.LayerParameter[nn.HiddenLayerNum].LayerParameter[i])
//		nn.LayerParameter[nn.HiddenLayerNum].LayerParameter[i].dB = d.dB
//		nn.LayerParameter[nn.HiddenLayerNum].LayerParameter[i].dW.Update(d.dW)
...
//			parameterTemp.Cell[i][j] = nn.LayerParameter[layerNo+1].LayerParameter[i].W.Cell[0][j]
//		}
//	}
//
...
//
//		yHatMatrix :=YHat(temp, nn.LayerParameter[layerNo].LayerParameter[i])
...
//		}
//
//		nn.LayerParameter[layerNo].LayerParameter[i].dB =Average(finalDerivative)
//		nn.LayerParameter[layerNo].LayerParameter[i].dW.Update(ScalarMatrix(MatrixMultiplication(finalDerivative, TransposeMatrix(temp)), 1/float64(temp.Column)))
//
//	}
//
//
...
//		fmt.Printf("%v",NeuralNetwork.Attribution)
//		panic("format error")
//	}
//
...
//				NeuralNetwork.LayerParameter[j].LayerParameter[k].W.Update(MatrixSubtraction(NeuralNetwork.LayerParameter[j].LayerParameter[k].W, ScalarMatrix(NeuralNetwork.LayerParameter[j].LayerParameter[k].dW,alpha)))
//				NeuralNetwork.LayerParameter[j].LayerParameter[k].B -= NeuralNetwork.LayerParameter[j].LayerParameter[k].dB * alpha
//			}
//		}
//
//	}
